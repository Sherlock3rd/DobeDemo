...
- 全局 `GlobalHud` 在城市主界面显示钱、油、物资、英雄经验及赛车/推关/英雄/设置入口；打开建筑详情、赛车、推关、编队、英雄、帮派树、设置或战斗覆盖层时，HUD 会隐藏并 inert，关闭覆盖层后恢复。应用只允许一个 `activeOverlay`，各覆盖层互斥。
...
- 三车道自动前进：`↑/↓`、`W/S` 或触屏方向按钮切换车道，空格/触屏“加速”消耗氮气，追击关使用 `F`/触屏“开火”。
- 竞速关需要躲避路障并抢先冲线；追击关需要追近目标、保持同车道和射程后射击，目标会换道并还击。车辆耐久耗尽、目标逃脱或超时都会失败。
...
- 数值分别由 `equipment.config.json` 与 `racing.config.json` 管理，确定性的 100ms 固定步长引擎不读取随机数。
...
当前验收基线：69 个测试文件、714 项测试。最终本地构建资产为 `/DobeDemo/assets/index-ClwAyOWp.js` 与 `/DobeDemo/assets/index-D6A5oCnJ.css`。
