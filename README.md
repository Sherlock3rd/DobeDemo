...
- 全局 `GlobalHud` 在城市主界面以共享 SVG 图标显示钱、油、物资数量，并把账号总战力放在帮派按钮内；同时提供赛车/推关/英雄/设置入口。英雄经验保留在推关与英雄培养界面。打开建筑详情、赛车、推关、编队、英雄、帮派树、设置或战斗覆盖层时，HUD 会隐藏并 inert，关闭覆盖层后恢复。应用只允许一个 `activeOverlay`，各覆盖层互斥。
