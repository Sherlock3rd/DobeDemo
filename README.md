...
- 城市存档（主/子建筑等级、资源钱包、生产时间、激活生产建筑）、显式帮派等级/声望与章节领取记录均通过 `localStorage` 持久化；旧帮派存档会按已有声望一次性迁移出当前等级，此后声望不再自动升级。选中子槽、详情/确认页与动画属于本次面板会话，不持久化。
...
- 全局 `GlobalHud` 在城市主界面以共享 SVG 图标显示钱、油、物资数量，并把账号总战力放在帮派按钮内；同时提供左侧章节悬浮入口及赛车/推关/英雄/设置入口。英雄经验保留在推关与英雄培养界面。打开建筑详情、章节、赛车、推关、编队、英雄、帮派树、设置或战斗覆盖层时，HUD 会隐藏并 inert，关闭覆盖层后恢复。应用只允许一个 `activeOverlay`，各覆盖层互斥。
...
- 点击左下角章节悬浮按钮：查看当前职级章节剧情、任务进度和可领取奖励；点击任务的“前往”按钮可直接进入对应建筑、养成或玩法，全部任务完成并领取章节奖励后回到帮派树跨职级晋升。
- 帮派树内点击“晋升一级”：经验满足时提升一级；从 Lv.7/15/23/31/39/49 跨入下一职位时还会检查当前章节是否全部完成。
