...
- 英雄战役存档键继续使用 `dobe-adventure-progression-v1`，当前 persist 版本为 v4，保存英雄等级、共享经验、阵容、推关/赛车最高通关、英雄车枪、枪械强化、车辆配件、零件货币与两类挂机时钟；v1/v2/v3 旧存档保留原进度并补齐新的空养成结构，旧 `armor`/`turbo` 配件会分别迁移到保险杠/悬挂，坏存档在 rehydrate 时夹紧、去重并丢弃非法数据。
...
- 英雄等级只消耗共享英雄经验升级；枪械只消耗“零件”强化，最高 Lv.10，强化同时提高推关英雄 ATK 和追击枪战伤害。
- 每辆车拥有轮胎、引擎、保险杠、悬挂四个固定配件槽，配件只能安装到对应部位。配件绑定车辆而非英雄，因此换英雄驾驶时整套配件随车保留；配件等级最高 Lv.10，安装和升级会同时提高英雄战力及赛车的极速、加速度、抓地或耐久。
- 废车回收厂在帮派 Lv.8 解锁后开始挂机获得配件，主建筑 Lv.1–10 的产出间隔由 30 秒逐级缩短至 12 秒，离线最多结算 8 小时。仓库上限 40 件，满仓后的新配件自动回收为零件；不需要且未安装的配件可主动回收，返还品质基础值和 70% 已投入零件。
- 配件分为旧件、调校、精工、原型四种品质；产出序列确定且随废车回收厂等级开放更高品质，不读取随机数，便于存档复现和自动化测试。
...
- 追击关由装甲目标车和五辆护卫车组成六车移动队形，并完全关闭玩家、目标与护卫的氮气积攒、释放及尾流加速，只保留纯追击枪战。玩家普通攻击会持续自动开火；`F`/触屏按钮触发约 2.4 秒的攻速与伤害强化，之后进入 8 秒冷却，每次完成干净的空中特技会立即缩短 2.5 秒冷却。追击关玩家车辆耐久翻倍，目标与护卫会持续还击。护卫会主动封锁射界；玩家需要换道、突破或摧毁护卫，再以具有飞行时间的可见弹道攻击目标。枪口焰、曳光弹、命中火花、碎片、烟雾、爆炸、后坐与车辆受力共同提供射击反馈。
