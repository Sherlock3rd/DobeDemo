...
- 三种资源：钱、油、物资，均为非负整数；主 HUD 以图标只显示余额，不显示增量，并额外显示“全部已解锁英雄当前装备战力 + 全部已解锁主建筑已完成等级战力”的总战力。
- 每 10 秒结算一个生产 tick，离线最多结算 8 小时；不足一个 tick 的时间余量保留到下次。
- 生产建筑：修车厂与商业街产钱、加油站产油、金属加工厂产物资；废车回收厂独立挂机产出车辆配件，Clubhouse 本版不产出。
...
- 全局 `GlobalHud` 在城市主界面以共享 SVG 图标显示钱、油、物资数量和账号总战力，以及赛车/推关/英雄/设置入口；英雄经验保留在推关与英雄培养界面。打开建筑详情、赛车、推关、编队、英雄、帮派树、设置或战斗覆盖层时，HUD 会隐藏并 inert，关闭覆盖层后恢复。应用只允许一个 `activeOverlay`，各覆盖层互斥。
- 战斗退出需要二次确认；确认态会冻结战斗并阻止胜利/首通/经验发奖。Overlay 打开时标题获得焦点、背景变为 `inert`，关闭后焦点恢复到触发点。
- 英雄战役存档键继续使用 `dobe-adventure-progression-v1`，当前 persist 版本为 v5；旧档中超过最高英雄等级的枪械或配件会降到合法等级，并完整返还超出部分的逐级强化零件。
- Store 拒绝锁定英雄和非法帮派等级请求；配置及派生计算校验 safe integer、英雄 `defaultSlot` 与数值溢出。
- 建筑进度表示“当前主建筑等级这一阶段”的完成度；阶段完成后仍保留精确 `100%` 进度，并同时显示主建筑升级按钮。

### 公路争霸与英雄装备

- 英雄培养界面采用“左侧英雄名单 + 中央英雄与战力 + 右侧养成”的三栏结构，桌面弹窗和手机单层滚动布局共用“等级 / 车辆 / 枪械”三个清晰页签。车辆和枪械页会先展示该英雄的当前装备与养成入口，点击“更换车辆 / 更换枪械”才进入独立装备选择界面；选择页会标明当前装备、其他英雄占用或车库/武器库待命状态，完成切换后自动返回当前装备页。
- 英雄等级只消耗共享英雄经验升级；枪械和配件硬上限均为 Lv.50，实际强化等级不得超过账号内最高英雄等级。Lv.10 后采用递减成长，强化继续提高推关、竞速与追击实战属性。
- 每辆车拥有轮胎、引擎、保险杠、悬挂四个固定配件槽，配件只能安装到对应部位。配件绑定车辆而非英雄，因此换英雄驾驶时整套配件随车保留；仓库支持按旧件/调校/精工/原型品质一键回收，已安装配件会自动排除。
- 废车回收厂在帮派 Lv.8 解锁后开始挂机获得配件，主建筑 Lv.1–10 的产出间隔由 30 秒逐级缩短至 12 秒，单批数量由固定 1 件提升到 Lv.10 的 2–4 件，离线最多结算 8 小时。仓库上限 40 件，满仓后的新配件自动回收为零件。
- 配件分为旧件、调校、精工、原型四种品质；数量和品质均为真随机，等级越高则单批数量和高品质概率越高，Lv.10 原型概率为 25%。随机源可在自动化测试中注入，完整概率可在设置页查看。
...
- 点击“英雄”或 Thomas Shelby 头像：进入三栏英雄培养页；车辆/枪械页签会直接写出当前装备，点击独立的“更换”入口后再从车库或武器库切换装备。车辆详情下方固定展示轮胎、引擎、保险杠、悬挂四个槽位，点击槽位会进入与装备车一致的更深一层选择页，仅显示该部位可用配件；配件卡统一显示部位、品质、等级 Tag，并可安装、转移、升级、卸下或回收，当前枪械可消耗零件强化。
...
- 第一版建筑升级不实际消耗油或物资（成本结构保留三资源）；废车回收厂不生产三资源，而是通过独立挂机时钟产出车辆配件，Clubhouse 不配置产出。
