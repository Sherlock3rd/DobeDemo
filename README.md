...
- 在线每 10 秒获得 1 点声望，支持最多 8 小时离线收益；主 HUD 只显示当前资源余额，不显示增量。
- 1–50 级帮派树，共 7 个职位；声望作为帮派经验累计，玩家需在帮派树逐级点击“晋升一级”。
- 七个职位分别对应七章任务。跨入下一职位前必须完成当前章节；每章只出现本职位起始等级已开放的功能。
- 主 HUD 左侧悬浮章节入口展示当前章节。章节界面包含剧情、完成数、任务进度及手动领取；奖励包括帮派经验、英雄经验、零件与高品质配件，第一章末尾固定发放四部位完整史诗（紫色）套装。
...
- 五个竞速关统一为约 85 秒设计时长，路线由 3100 米逐步增加到 4200 米。公路使用矮橙色护栏表现普通路障，跳台则使用高耸的青色倾斜坡面、黄色导向纹、发光侧栏和入口门架，确保高速下可以提前辨认。普通跳台滞空约 1.5–2.3 秒，为上一版长滞空的一半；玩家与 AI 都会飞坡、翻滚并落地。成功特技一次补充大量氮气，成为氮气主要来源；自然回复降至初版一半，漂移和尾流只提供辅助补充。任意车辆若以偏离直立姿态 0.2 圈以上的角度落地，会瞬间降至约 62% 车速；玩家还会损失耐久。
...
- 三名英雄改为浴血黑帮姓名库：主角 Thomas Shelby（Tommy）、Arthur Shelby（Arthur）与 Polly Gray（Aunt Polly）。初始向 Thomas Shelby 发放并装备“灰狐旧改车”和“铆钉冲锋枪”；帮派树在 Lv.8/16/32/40 继续投放车辆，在 Lv.12/24/32/40 继续投放枪械，共 5 车、5 枪。
...
- 点击左侧章节悬浮按钮：查看当前职级章节剧情、任务进度和可领取奖励；章节全部完成后回到帮派树跨职级晋升。
