...
- 三车道高速自动前进：点按 `←/→`、`A/D` 或触屏方向按钮换道，长按方向进入物理漂移；空中继续按方向可完成翻滚特技，空格/触屏“氮气”释放加速，追击关使用 `F`/触屏“火力强化”。
- 氮气固定为三格，所有车辆开局均为 0；至少蓄满一格才能释放，普通释放消耗一格并获得 1.8 秒冲刺。满三格时双击会耗尽全部氮气，触发 3.2 秒超级加速和最高约 26 米的飞跃；AI 同样会积攒、普通释放，并有部分车辆保留满格发动超级飞跃。
...
- 追击关由装甲目标车和五辆护卫车组成六车移动队形。玩家普通攻击会持续自动开火；`F`/触屏按钮触发约 2.4 秒的攻速与伤害强化，之后进入 8 秒冷却。追击关玩家车辆耐久翻倍，目标与护卫会持续还击。护卫会主动封锁射界；玩家需要换道、突破或摧毁护卫，再以具有飞行时间的可见弹道攻击目标。枪口焰、曳光弹、命中火花、碎片、烟雾、爆炸、后坐与车辆受力共同提供射击反馈。
