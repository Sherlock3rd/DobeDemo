...
- 英雄战役存档键继续使用 `dobe-adventure-progression-v1`，当前 persist 版本为 v3，保存英雄等级、共享经验、阵容、推关/赛车最高通关、英雄车枪、枪械强化、车辆配件、零件货币与两类挂机时钟；v1/v2 旧存档保留原进度并补齐新的空养成结构，坏存档在 rehydrate 时夹紧、去重并丢弃非法数据。
...
- 每辆车拥有动力、装甲、轮胎、涡轮四个独立配件槽。配件绑定车辆而非英雄，因此换英雄驾驶时整套配件随车保留；配件等级最高 Lv.10，安装和升级会同时提高英雄战力及赛车的极速、加速度、抓地或耐久。
...
- 氮气固定为三格，所有车辆开局均为 0；至少蓄满一格才能释放，普通释放消耗一格并获得 1.8 秒冲刺。满三格时双击会耗尽全部氮气，触发 3.2 秒超级加速和最高约 26 米的飞跃；竞速时落后领跑者超过 4 米便会按距离差额外补氮，最深落后时额外补充 18 点/秒，AI 会在落后时更积极释放普通氮气，保留型 AI 则会蓄满发动超级飞跃。
- 竞速关为玩家加六辆 AI 的七车同场。每辆车都有独立质量、速度、横向惯性、偏航、耐久与悬挂参数；前后追撞会按质量与相对速度交换动量，后车减速、前车受推加速，氮气冲撞还会产生额外破坏。
- 公路包含可见路障与上扬跳台；腾空时间约为初版三倍，玩家与 AI 都会飞坡、翻滚并落地。成功特技一次补充大量氮气，成为氮气主要来源；自然回复降至初版一半，漂移和尾流只提供辅助补充。任意车辆若以偏离直立姿态 0.2 圈以上的角度落地，会瞬间降至约 62% 车速；玩家还会损失耐久。
- 追击关由装甲目标车和五辆护卫车组成六车移动队形，并完全关闭玩家、目标与护卫的氮气积攒、释放及尾流加速，只保留纯追击枪战。玩家普通攻击会持续自动开火；`F`/触屏按钮触发约 2.4 秒的攻速与伤害强化，之后进入 8 秒冷却。追击关玩家车辆耐久翻倍，目标与护卫会持续还击。护卫会主动封锁射界；玩家需要换道、突破或摧毁护卫，再以具有飞行时间的可见弹道攻击目标。枪口焰、曳光弹、命中火花、碎片、烟雾、爆炸、后坐与车辆受力共同提供射击反馈。
...
- 点击“英雄”或 Thomas Shelby 头像：进入三栏英雄培养页；车辆/枪械页签会直接写出当前装备，点击独立的“更换”入口后再从车库或武器库切换装备。当前车辆可安装/升级/卸下四类配件并回收闲置配件，当前枪械可消耗零件强化。
