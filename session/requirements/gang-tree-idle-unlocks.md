...
- 每级需要 30 声望。
- 在线每 10 秒获得 1 声望（HUD 显示 `+1 声望/10秒`）；不足 10 秒不产出并保留时间余量。
...
- 照片墙每一层最多保留 2 个空置席位，避免空位挤压有效人物与建筑照片。
- 人物头像图集按 4×2 的正方形切片比例显示；照片墙卡片与 Thomas 当前席位均保持 1:1，不允许随卡片宽度拉伸。
...
- 达到阈值后地块立即切换为对应建筑 Lv.1 模型，并开放渐进式子建筑与三资源经营升级闭环（六类建筑主等级统一 Lv.10；修车厂 5 个槽、其余 10 个槽随主等级逐槽解锁，未解锁槽在 UI/3D 完全隐藏；一个公用子升级按钮逐级建设，追平后经独立确认页升主建筑；修车厂目标 Lv.2–5 无外部门槛/Lv.6–10 受 Clubhouse 约束，其余四类目标 Lv.2–5 受修车厂约束/Lv.6–10 受 Clubhouse 约束，Clubhouse 只受自身约束；详见 `docs/superpowers/specs/2026-07-24-progressive-building-upgrade-flow-design.md`）。
...
5. [x] 声望按每 10 秒 +1 自动获得（原每秒 5 点已改版），每 30 点提升一级，1470 封顶。
6. [x] 本地持久化、完整 10 秒 tick 余量、离线 8 小时上限、系统时间异常和满级边界均有测试。
...
- 不包含帮派成员、任务、战斗或分支技能点。
- 建筑升级已从旧“固定顺序碎片闭环”“独立子建筑闭环”演进为渐进式子建筑与三资源（钱/油/物资）经营：六类建筑主等级统一 Lv.10；子建筑槽位随主等级逐槽解锁（修车厂 5 槽、其余 10 槽），未解锁槽在 UI/3D 完全隐藏；一个公用子升级按钮逐级建设，全部追平后经独立确认页扣费升主建筑；修车厂目标 Lv.2–5 无外部门槛、Lv.6–10 受 Clubhouse 约束，其余四类目标 Lv.2–5 受修车厂约束、Lv.6–10 受 Clubhouse 约束，Clubhouse 仅受自身约束；建筑战力仅用于展示。新账号与重置钱包固定为 `钱 10000 / 油 0 / 物资 0`；升级消耗资源（第一版仅消耗钱），仍无付费加速或手动领取。子建筑等级、资源钱包、生产时间与激活生产建筑持久化于 `dobe-city-progression-v1`（Zustand persist 版本 3，v2→v3 一次性为被隐藏旧子槽退款）。设置面板含“解锁帮派树”“钱/油/物资各 +10000”两个调试动作与二次确认的重置。`src/config/economy.config.json`（schema 版本 2，含 `buildingPowerById`）冻结后续配置 EXE 的 JSON 契约，本次不实现 EXE。
