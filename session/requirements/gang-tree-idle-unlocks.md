...
- 达到阈值后地块立即切换为对应建筑 Lv.1 模型，并开放独立子建筑与三资源经营升级闭环（修车厂 5 个子建筑、其余 10 个，子建筑从 Lv.0 自由升级，追平后升主建筑，Clubhouse Lv.10/其余 Lv.5 上限；详见 `docs/superpowers/specs/2026-07-23-independent-subbuilding-economy-design.md`）。
...
- 不包含帮派成员、任务、战斗或分支技能点。
- 建筑升级已从旧“固定顺序碎片闭环”演进为独立子建筑与三资源（钱/油/物资）经营：子建筑从 Lv.0 自由升级、追平后升主建筑、Clubhouse Lv.10 其余 Lv.5，且非 Clubhouse 目标等级不得高于 Clubhouse 当前等级；升级消耗资源（第一版仅消耗钱），仍无付费加速或手动领取。子建筑等级、资源钱包、生产时间与激活生产建筑持久化于 `dobe-city-progression-v1`（版本 2）。`src/config/economy.config.json` 冻结后续配置 EXE 的 JSON 契约，本次不实现 EXE。
